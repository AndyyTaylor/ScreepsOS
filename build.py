...
with open('src/config.py', 'r') as f:
    for line in f:
        if 'VERSION' in line and 'SIGN' not in line:
            line = line.replace('js_global.VERSION = ', '')
            version = int(line) + 1
            print(version)
            line = 'js_global.VERSION = ' + str(version) + '\n'
        new_file += line
...
def install_env(config):
    ...
    if config.enter_env:
        ...
        if not os.path.exists(os.path.join(env_dir, 'bin', 'transcrypt')) and not os.path.exists(
            ...
            install_args = ['pip3', 'install', '-r', requirements_file]
